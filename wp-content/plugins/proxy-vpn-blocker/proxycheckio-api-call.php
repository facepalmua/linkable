...
function proxycheck_function( $visitor_ip, $asn_check ) {
	...
	if ( time() >= $pvb_transient_exploded[0] ) {
		...
		} else if ( ! empty( get_option( 'pvb_proxycheckio_Custom_TAG_field' ) ) ) {
		...
		if ( 'yes' == $decoded_json->$visitor_ip->proxy ) {
			...
			if ( isset( $decoded_json->$visitor_ip->risk ) ) {
				if ( 1 == $asn_check ) {
					$array = array( '1', $decoded_json->$visitor_ip->country, $decoded_json->$visitor_ip->continent, $decoded_json->$visitor_ip->risk, $decoded_json->$visitor_ip->type );
				} else {
					$array = array( '1', 'null', 'null', $decoded_json->$visitor_ip->risk, $decoded_json->$visitor_ip->type );
				}
				return $array;
			} else {
				if ( 1 == $asn_check ) {
					$array = array( '1', $decoded_json->$visitor_ip->country, $decoded_json->$visitor_ip->continent, 'null', $decoded_json->$visitor_ip->type );
				} else {
					$array = array( '1', 'null', 'null', $decoded_json->$visitor_ip->risk, $decoded_json->$visitor_ip->type );
				}
				return $array;
			...
			// A proxy has not been detected "0" but still check if country or continent is blocked, return true and don't IP cache this.
			if ( 1 == $asn_check ) {
				$array = array( '0', $decoded_json->$visitor_ip->country, $decoded_json->$visitor_ip->continent, 'null', 'null' );
			} else {
				$array = array( '0', 'null', 'null', 'null', 'null' );
			}
			return $array;
		}
	} else {
		if ( $pvb_transient_exploded[1] = '0' ) {
			$array = array( '1', 'null', 'null', 'null', 'null' );
			return $array;
		} else {
			$array = array( '0', 'null', 'null', 'null', 'null' );
			return $array;
