...
if ( !empty( $get_api_key ) ) {
	...
	if ( isset( $current_blacklist->status ) && 'denied' === $current_blacklist->status ) {
		...
		$html .= '<h1>' . __( 'Proxy &amp; VPN Blocker proxycheck.io Blacklist', 'proxy-vpn-blocker' ) . '</h1>' . "\n";
		...
		$html .= '<h2>' . __( 'proxycheck.io Blacklist', 'proxy-vpn-blocker' ) . '</h2>' . "\n";
		$html .= '<p>' . __( 'Here you can view and edit your proxycheck.io Blacklist.', 'proxy-vpn-blocker' ) . '</p>' . "\n";
