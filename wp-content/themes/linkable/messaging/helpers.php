...
function employer_get_latest_message($project_id, $author_id ) {
	global $wpdb;
	$msg_table = $wpdb->prefix . 'la_private_messages';
  $current_user = get_current_user_id();
	$sql = $wpdb->prepare("SELECT message FROM {$msg_table}
											WHERE project_id = {$project_id}
											AND author_id = {$author_id}
											Order BY send_date DESC LIMIT 1");
	$qry = $wpdb->get_results( $sql );
	if (strpos($qry[0]->message, '[@linkable_status@]') !== false) {
		return 'Contract Updated';
	}
	return stripslashes($qry[0]->message);
}
...
function get_user_messages( $user_id = 0  ) {
  ...
  $current_user = get_current_user_id();
	if( ae_user_role() == EMPLOYER ) {
		...
		$results_arr = array();
		$sql = $wpdb->prepare("SELECT ID, post_title FROM {$wpdb->posts} WHERE post_author = %d AND post_type = 'project'", $current_user);
		$qry = $wpdb->get_results( $sql );
		foreach ($qry as $post_item) {
			$sql = $wpdb->prepare("SELECT * FROM {$msg_table} INNER JOIN (SELECT MAX(send_date) AS MaxDateTime FROM {$msg_table} WHERE project_id = {$post_item->ID} GROUP BY author_id) groupedtt
													WHERE project_id = {$post_item->ID}
													AND send_date = groupedtt.MaxDateTime
													Order BY send_date DESC");
			$qry = $wpdb->get_results( $sql );
			$results_arr[] = $qry;
		}
		$return = array();
		foreach ($results_arr as $res_arr_item) {
			foreach ($res_arr_item as $key => $value) {
				if ($value->id) {
					$value->ID = $value->project_id;
					$value->post_title = get_the_title($value->project_id);
		...
		for ($i = 0; $i < count($return); $i++){
			for ($j = $i + 1; $j < count($return); $j++) {
				if ($return[$i]->send_date < $return[$j]->send_date) {
					$temp = $return[$j];
					$return[$j] = $return[$i];
					$return[$i] = $temp;
				}
		...
		$sql = $wpdb->prepare("
            ...
            FROM (SELECT * FROM {$msg_table} ORDER BY send_date desc) x
            LEFT JOIN {$wpdb->posts} ps
		...
		$results_arr = array();
		$sql = $wpdb->prepare("SELECT ID, post_title FROM {$wpdb->posts}");// WHERE post_status = 'publish'");
		$qry = $wpdb->get_results( $sql );
		foreach ($qry as $post_item) {
			$sql = $wpdb->prepare("SELECT * FROM {$msg_table} WHERE author_id = {$current_user} AND project_id = {$post_item->ID} ORDER BY send_date DESC LIMIT 1");
			$message = $wpdb->get_results( $sql );
			foreach ($message as $ms_item) {
				$ms_item->ID = $ms_item->project_id;
				$ms_item->post_title = get_the_title($ms_item->project_id);
				if ($ms_item->status == 'unread') {
					if ($ms_item->sender_id == $current_user) {
						$ms_item->status = 'read';
					}
				}
				$results_arr[] = $ms_item;
		...
		for ($i = 0; $i < count($results_arr); $i++){
			for ($j = $i + 1; $j < count($results_arr); $j++) {
				if ($results_arr[$i]->send_date < $results_arr[$j]->send_date) {
					$temp = $results_arr[$j];
					$results_arr[$j] = $results_arr[$i];
					$results_arr[$i] = $temp;
				}
			}
		}
...
function is_there_new_messages($return_bool = false) {
	global $wpdb;
	$msg_table = $wpdb->prefix . 'la_private_messages';
  $current_user = get_current_user_id();
	...
	if( ae_user_role() == EMPLOYER ) {
		...
		$current_user_projects = new WP_Query(
				array(
					'posts_per_page' => -1,
					'post_type' => PROJECT,
					'is_author'        => true,
		...
		$current_user_projects = $current_user_projects->get_posts();
		foreach ($current_user_projects as $project) {
			$project_id = $project->ID;
			$sql = $wpdb->prepare("SELECT id
							FROM {$msg_table}
							WHERE project_id = {$project_id} AND sender_id <> {$current_user} AND status = 'unread'; ");
			$qry = $wpdb->get_results( $sql );
			if( is_array( $qry) ) {
				if (count ( $qry ) > 0 ) {
					if ($return_bool) {
		...
		$sql = $wpdb->prepare("SELECT id
						FROM {$msg_table}
						WHERE author_id = {$current_user} AND sender_id <> {$current_user} AND status = 'unread' ");
		$qry = $wpdb->get_results( $sql );
		if( is_array( $qry) ) {
			if (count ( $qry ) > 0 ) {
				if ($return_bool) {
...
function is_messages_exists($project_id){
	global $wpdb;
	$msg_table = $wpdb->prefix . 'la_private_messages';
  $current_user = get_current_user_id();
	...
	$sql = $wpdb->prepare("SELECT id
					FROM {$msg_table}
					WHERE project_id = {$project_id} AND author_id = {$current_user}; ");
	$qry = $wpdb->get_results( $sql );
	if( is_array( $qry) ) {
		if (count ( $qry ) > 0 ) {
			return true;
...
function la_mark_project_read( $project_id, $author_id ) {
	...
	//$message = $args['project_id'] . ' ' . $args['author_id'];
	//wp_mail( 'm.yurchenko.web.dev.@mnail.com', 'LOG DATA', $message );
