...
require_once dirname( __FILE__ ) . '/color-hash.php';
/**
...
class LAPrivateMessaging{
	...
	public function load_assets_for_dashboard(){
		if( is_page_template( 'page-messages.php') ) {
			...
			wp_enqueue_script( 'messages_dashboard', get_stylesheet_directory_uri() . '/messaging/assets/messages.js?v=11.1', array('jquery'), null, true);
	 ...
	 */
	public function create_message( $args, $raw_msg = false, $send_status = false ) {
		...
		$date = current_time( 'timestamp' )-60;//current_time( 'mysql' )
		$date = date('Y-m-d H:i:s', $date);
		if (!$send_status) {
			$sql =
				"SELECT * FROM $this->tbl_messages
						WHERE `project_id` = $project_id AND
						message = '$message' AND send_date >= '$date'";
			$return = $this->db->get_results( $sql );
			//wp_mail( 'm.yurchenko.web.dev.@mnail.com', 'LOG DATA SQL', $sql . json_encode($return) . $return[0]->id);
			if ($return[0]->id) {
				return false;
			}
		}
...
//		$status = $this->db->update( $this->tbl_messages, $data, $where, ['%s'], ['%d', '%d'] );
		...
		//wp_mail( 'm.yurchenko.web.dev.@mnail.com', 'LOG DATA SQL', $sql );
