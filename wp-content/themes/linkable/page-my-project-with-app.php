<?php
/**
 * Template Name: My Project with PENDING Applications
 */
...
if ($user_role == FREELANCER) {
	$freelance_msg ='Visit the <a href="'.get_home_url().'/projects/">Project Marketplace</a> and search for content you can link to. Then submit your application!</span>';
}
...
?>
    ...
    <style>
        ...
        .employer-project-nav ul li:nth-child(2) a {
            color: #1aad4b;
            border-bottom: 1px solid #1faa49;
        }

        .project-sub-nav li:nth-child(2) a {
            color: white;
            font-weight: bold;
        }
                                    ...
                                    <div class="current-employer-project">
                                        <div class="fre-table">
                                            ...
                                            <div class="fre-current-table-rows">
												...
												//echo 'num pending: ' . $with_pending_app;


												$count = 0;

												if ( $with_pending_app == 0 ) {
													echo 'Any project with pending contracts would show up here. However, it currently looks like you do not have any for us to display here.';
                                                ...
                                                <div class="fre-table-row active-app">
                                                    <div class="fre-table-row-inner">
                                                        ...
                                                        <div class="project-description no-underline attribute">
                                                            ...
                                                            <p><?php $terms  = get_the_terms( $convert->ID, 'project-follow-type' );
																...
																//echo count(array_intersect($follow_type,$no_pref));
                                                    ...
                                                    <div class="project-footer-bar <?= $parent_status; ?>">
														...
														if ( $parent_status == 'publish' ) {
															...
															echo '<strong>Project Status:</strong> <span class="italic">Published and viewable for authors to apply.</span>';
														} else if ( $parent_status == 'deleted' || $parent_status == 'cancelled' ) {
															echo '<strong>Project Status:</strong> <span class="italic">This project was unpublished or has expired and will not receive any offers from new authors.</span>';
														} else {
															echo '<strong>Project Status:</strong> <span class="italic">Published and viewable for authors to apply.</span>';
														}
														...
														//
														//															if ( $parent_status != 'deleted' && $parent_status != 'cancelled' ) {
														//																echo '<span class="expires-wrap days-left"><i class="fa fa-calendar-alt"></i>Expires in <span>' . $time_left . '</span> days</span>';
														//															}

														?>

                                                        <span class="hidden-id"><?php echo $convert->ID; ?></span>
														<?php if ( $parent_status != 'deleted' ) { ?>
										...
										<?php }
											...
											$no_result_current = 'Any project with pending contracts would show up here. However, it currently looks like you do not have any for us to display here.';
										}
										?>
                                        </div>
                                    </div>
