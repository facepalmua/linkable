 ...
 */
function et_less2css($variables = array()){
	$less = file_get_contents(dirname(__FILE__) . '/customization.less');
	$lesslib = new lessc();
	$lesslib->setVariables($variables);
	$css = $lesslib->compile($less);
	// save the customization css
	update_option( 'ae_less_customize', $css );
	return $css;
}

function et_mobile_less2css($variables = array()){
	$less = file_get_contents(dirname(__FILE__) . '/mobile-customization.less');
	$lesslib = new lessc();
	$lesslib->setVariables($variables);
	$css = $lesslib->compile($less);
	// save the customization css for mobile
	update_option( 'ae_mobile_less_customize', $css );
	return $css;
}

function et_render_less_style() {
	et_enqueue_customize_font();
	echo '<style id="css-customize">';
	echo get_option('ae_less_customize');
	echo '</style>';
	do_action('print_define_less');
}

function et_mobile_render_less_style() {
	et_enqueue_customize_font();
	echo '<style id="mobile-customization">';
	echo get_option('ae_mobile_less_customize');
	echo '</style>';
}