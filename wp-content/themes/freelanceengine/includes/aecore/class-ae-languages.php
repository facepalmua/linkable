...
class AE_Language extends AE_Base
{
    ...
    public function get_language_list() {
         ...
         */
        if (defined('THEME_LANGUAGE_PATH')) $custom_langs = get_available_languages(THEME_LANGUAGE_PATH);
        else $custom_langs = array();

        $default_langs = get_available_languages(DEFAULT_LANGUAGE_PATH);
        foreach ($default_langs as $key => $value) {
            if (!in_array($value, $custom_langs)) $custom_langs[] = $value;
        }
    ...
    function save_language() {
        ...
        foreach ($language_files as $type => $mo) {
            ...
            if ($type == 'mo') {
                 // mo file
                if (in_array($selected_lang, get_available_languages(THEME_LANGUAGE_PATH))) $mo->import_from_file(THEME_LANGUAGE_PATH . '/' . $selected_lang . '.mo');
                else $mo->import_from_file(DEFAULT_LANGUAGE_PATH . '/' . $selected_lang . '.mo');
            } else {
                 // po file
    ...
    function load_translation_form() {
        ...
        */
        ...
        if (in_array($lang_name, $langArr)) {
            if (in_array($lang_name, get_available_languages(THEME_LANGUAGE_PATH))) $mo->import_from_file(THEME_LANGUAGE_PATH . '/' . $lang_name . '.mo');
            else $mo->import_from_file(DEFAULT_LANGUAGE_PATH . '/' . $lang_name . '.mo');
