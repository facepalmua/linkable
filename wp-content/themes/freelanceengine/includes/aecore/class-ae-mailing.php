...
class AE_Mailing extends AE_Base {
	...
	function forgot_mail( $user_id, $key ) {
		...
		$message .= '<p>It looks like you just sent a request to recover the password associated with your Link-able account. If you did not make this request, please ignore this email. Otherwise, click the link below to create your new password:</p>';
		...
		$activate_url = '<p><a href="https://link-able.com/reset-pass/?user_login='.$user_login.'&key=' . $key . '">Create new password</a></p>';
		
		$message .= $activate_url;
		
		$message .= "<p>Cheers!</p><p>The Link-able Team</p>";
	...
	function new_post_alert( $post ) {
		...
		$message = sprintf( __( "<p>Hi,</p><p> Have a new post on your site. You can review it here: %s </p>", ET_DOMAIN ),
			"<a href='" . get_permalink( $post ) . "' target='_Blank'>" . get_the_title( $post ) . "</a>" );
	...
	public function wp_mail( $to, $subject, $content, $filter = array(), $headers = '' ) {
		if ( $headers == '' ) {
			...
			// $headers = 'MIME-Version: 1.0' . "\r\n";
			// $headers.= 'Content-type: text/html; charset=utf-8' . "\r\n";
			$headers .= "From: " . get_option( 'blogname' ) . " < " . 'noreply@link-able.com' . "> \r\n";
		}
