...
class AE_Comments
{
    ...
    public function __construct($comment_type, $meta_key = array() , $post_type = '') {
        ...
        $this->duplicate_error = __('Already added.', ET_DOMAIN);
        $this->limit_time =  120;
    }
     ...
     * @param string $thumb is type of thubnail
     * @param bool $merge_post
     * @param bool $merge_author
     *
    ...
    function convert($comment, $thumb = 'thumbnail', $merge_post = true, $merge_author = true ){
        ...
        */
        ...
        if(isset($comment->comment_author_email)) {
            $comment->avatar_user = get_avatar($comment->comment_author_email,'','http://www.gravatar.com/avatar/');
            unset($comment->comment_author_email);
        }else {
            $comment->avatar_user = get_avatar('','','http://www.gravatar.com/avatar/');
        }
    ...
    */
    public function insert($args) {
        ...
        if(!$this->duplicate) {
            ...
            if (!empty($comments)) {
                if(isset($args['type'])) {
                    return new WP_Error('duplication', $this->duplicate_error);
                }else {
