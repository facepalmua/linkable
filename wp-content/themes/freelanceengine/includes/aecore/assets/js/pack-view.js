...
(function(Models, Views, Collections, $, Backbone) {
	...
	Views.PackForm = Backbone.View.extend({
		...
		events: {
			...
			'change form.edit-plan input[name=et_price]' : 'checkUniquePrice',
		...
		checkUniquePrice : function(event){
			var $form = this.$el,
				$button = $form.find('.engine-submit-btn'),
				view = this,
				$price = $form.find('input[name="et_price"]'),
				collection = this.model.collection;

			if(view.model.get('post_type') == 'pack'){
				if(parseFloat($price.val()) == 0){
					var flag = false;
					$.each(collection.models, function(key, pack){
						if(parseFloat($price.val()) == parseFloat(pack.get('et_price'))){
							flag = true;
						}
					});
					if(flag){
                		...
                		$price.parent().append('<label for="et_price" class="error">'+ ae_globals.uniquePrice_text +'</label>');
	...
	Views.Pack = Backbone.View.extend({
		...
		events: {
			...
			'change form.add-pack-form input[name=et_price]' : 'checkUniquePrice',
		...
		checkUniquePrice : function(event){
			...
			if($form.data('option-name') == 'pack'){
				if(parseFloat($price.val()) == 0){
					...
					if(flag){
						$price.addClass('error_price');
                		$price.parent().find('label').remove();
                		$price.parent().append('<label for="et_price" class="error" style="display:block;">'+ ae_globals.uniquePrice_text +'</label>');
                		$button.prop('disabled', true);
                		return false;
					}else{
						$price.removeClass('error_price');
                		$price.parent().find('label').remove();
                		$button.prop('disabled', false);
                		return true;
					}
