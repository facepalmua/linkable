...
function ae_user_package_info($user_ID) {
    ...
    ?>
            ...
            <?php
                ...
                if ($order->post_status == 'publish') printf(__("The <strong>%s</strong> package you purchased has %d post(s) left", ET_DOMAIN) , $package->post_title, $number_of_post);
                if ($order->post_status == 'pending') printf(__("The <strong>%s</strong> package you purchased has %d post(s) left. Your posted post is pending until the payment is processed.", ET_DOMAIN) , $package->post_title, $number_of_post);
    ?>
            </p>
...
/**
 * rennder total user had package info
 * @param Integer $user_ID the user_ID want to render
 *
 * @package AE Package
 * @category payment
 ...
 * @author ThanhTu
 */
function ae_user_get_total_package($user_ID){
    if (!$user_ID) return;
    global $ae_post_factory;
    $ae_pack = $ae_post_factory->get('pack');
    $packs = $ae_pack->fetch();
    ...
    foreach ($packs as $package) {
        ...
        if (isset($package_data[$sku])) {
            ...
            if(isset($package_data[$sku]['qty']) && $package_data[$sku]['qty'] > 0) {
                if( $package->post_type == 'pack'){
                    $order = get_post($orders[$sku]);
                    if (!$order || is_wp_error($order) || !in_array($order->post_status, array('publish'))) continue;
                    $number_of_post = $package_data[$sku]['qty'];
                    $total_package = $total_package + $number_of_post;
                }
            }
        }
    }
