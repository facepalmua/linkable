...
class AE_Users {
	...
	public function forgot( $user_data ) {
		...
		return array(
			'success' => true,
			'msg'     => __( "Please check your mailbox to get password reset link.", ET_DOMAIN )
		);
	}
	...
	function check_activation_key( $key, $login ) {
		...
		if ( empty( $key ) || ! is_string( $key ) ) {
			return new WP_Error( 'invalid_key', __( '1Invalid Activation Key.', ET_DOMAIN ) );
		}

		if ( empty( $login ) || ! is_string( $login ) ) {
			return new WP_Error( 'invalid_key', __( '2Invalid User Name', ET_DOMAIN ) );
		}
		...
		if ( empty( $user ) ) {
			return new WP_Error( 'invalid_key', __( '3Invalid Activation Key.', ET_DOMAIN ) );
		}
	...
	function resetpass( $args ) {
		try {
			...
			if ( empty( $args['user_key'] ) ) {
				throw new Exception( __( "Key is Empty", ET_DOMAIN ) );
			}
			...
			return array(
				'success' => true,
				'msg'     => __( 'Your password has been successfully updated.', ET_DOMAIN )
			);
