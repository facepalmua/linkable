...
class AE_Page extends AE_Base {
	...
	public function print_scripts() {
		...
		$this->add_script( 'lib-nested-sortable', ae_get_url() . '/assets/js/jquery.nestedSortable.js', array(
			'jquery',
			'jquery-ui-sortable'
		) );
		// tam thoi add de xai
		...
		$vars            = apply_filters( 'ae_admin_globals', array(
			...
			'uniquePrice_text' => __( "The free package already existed.", ET_DOMAIN )
		) );
		wp_localize_script( 'appengine', 'ae_globals', $vars ); // this use for admin section
	}
...
class AE_Menu extends AE_Page {
	...
	function action_sync() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json( array(
				'success' => false,
				'msg'     => __( "You do not have permission to change option.", ET_DOMAIN )
			) );
		}
	...
	*/
	...
	public function admin_bar_menu() {
		...
		if ( ! current_user_can( 'manage_options' ) || ! apply_filters( 'ae_admin_bar_menu', true ) ) {
			return false;
		}
