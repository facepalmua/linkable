...
abstract class  ET_PaymentVisitor {
	...
	function add_return_param() {
		...
		$link       = et_get_page_link( 'process-payment', array(
			'paymentType' => $this->_payment_type,
...
class ET_PaypalVisitor extends ET_PaymentVisitor {
	...
	function setup_checkout( ET_Order $order ) {
		...
		$api        = $payment->get_api();
		$url        = $settings['return'];
		$cancel_url = et_get_page_link( 'cancel-payment' );//$settings['cancel'];
		...
		if ( ! empty ( $pro ) ) {
			...
			if ( isset( $api['invoice_prefix'] ) ) {
				$products .= "&invoice=" . trim( $api['invoice_prefix'] ) . $order['ID'];
			}
	...
	function do_checkout( ET_Order $order ) {
		 ...
		 * @author Dakachi
		 */
		if ( 'waiting' === $paymentStatus ) {
			return $this->do_checkout_get_back( $order, $paymentStatus );
		}
	 ...
	 * @author Dakachi
	 */
	function do_checkout_get_back( ET_Order $order, $paymentStatus ) {
...
class ET_2COVisitor extends ET_PaymentVisitor {
	...
	function setup_checkout( ET_Order $order ) {
		...
		$return = "&x_receipt_link_url=" . $this->_settings['return'] . "&order-id=" . $order['ID'];
