	...
	if ( $in_same_term || ! empty( $excluded_terms ) ) {
		...
		if ( ! empty( $excluded_terms ) && ! is_array( $excluded_terms ) ) {
			...
			if ( false !== strpos( $excluded_terms, ' and ' ) ) {
				_deprecated_argument( __FUNCTION__, '3.3', sprintf( __( 'Use commas instead of %s to separate excluded terms.', ET_DOMAIN ), "'and'" ) );
...
if ( ! function_exists( 'et_get_attachment_data' ) ):
	...
	function et_get_attachment_data( $attach_id, $size = array() ) {
		...
		$data['size'] = sprintf( '%.2f ' . $s[ $e ], ( $bytes / pow( 1024, floor( $e ) ) ) );
...
function et_get_mobile_header( $name = null ) {
	...
	if ( '' == locate_template( $templates, true ) ) //load_template( ABSPATH . WPINC . '/theme-compat/header.php');
	{
...
function et_get_mobile_footer( $name = null ) {
	...
	if ( '' == locate_template( $templates, true ) ) //load_template( ABSPATH . WPINC . '/theme-compat/footer.php');
	{
		load_template( $templates );
	}
}
...
function is_social_connect_page() {
	...
	if ( strpos( $social_connect_page, $current_page_slug ) ) {
		return true;
	}
	if ( $page_connect_id ) {
...
/**
 * Deregister Javascript on backend
 * @author ThanhTu
 */
function deregister_script_admin( $hook ) {
	global $pagenow;
	// Disable Script of Visual Composer in page admin.php
	if ( $pagenow == 'admin.php' && is_admin() ) {
		wp_deregister_script( 'vc_accordion_script' );
	}
	// Conflict with Visual Composer in edit page
	if ( $pagenow == 'post.php' && is_admin() ) {
		wp_deregister_script( 'appengine' );
	}
}

add_action( 'admin_enqueue_scripts', 'deregister_script_admin', 10, 3 );


/**
 * Change user role after install sample data
 */

function et_change_user_role() {
	$employers   = array( 'jazminewinton', 'kermitlawrence', 'hatruong', 'dorisclarke' );
	$freelancers = array( 'hamletavakian', 'taikiyamada', 'fadilaahmed', 'morganmelvin', 'martinharvey' );

	foreach ( $employers as $key => $value ) {
		$user_data = get_user_by( 'login', $value );
...
if( !function_exists( 'ae_gg_recaptcha') ) {
    ...
    *
    **/
    function ae_gg_recaptcha($container = ''){
...
/**
 * validate recaptcha in ajax
 * @author danng
 * @version 1.8.4
 * @param   string $captcha_response
 * @return  boolean
 */
function ae_verify_captcha(  $captcha_response) {

	$remote_ip = $_SERVER['REMOTE_ADDR'];

	$enable = (int) ae_get_option('gg_captcha', false );
	//$site_key = ae_get_option('gg_site_key') ;
	$secret_key = ae_get_option('gg_secret_key') ;
	...
	return new WP_Error( 'gglcptch_error', __('Captcha Invalid',ET_DOMAIN) );
}