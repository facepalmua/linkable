...
?>
	<div class="et-main-content order-container" id="">
		<div class="search-box et-member-search">
			<form action="">
				...
				<span class="et-search-input">
					<input type="text" class="et-input order-search" name="keyword" placeholder="<?php _e("Search post...", ET_DOMAIN); ?>">
		...
		<div class="et-main-main no-margin clearfix overview list">			
			<div class="title font-quicksand"><?php _e('All Orders', ET_DOMAIN) ?></div>
			<!-- order list  -->
        ...
        if ($orders->max_num_pages > 1) { ?>
				<button class="et-button btn-button load-more" >
					<?php _e('More Payments', ET_DOMAIN) ?>
				</button>
			<?php
        } ?>	        			
		</div>
    ...
    function sync_order() {
        ...
        do_action('ae_after_update_order', $order_id);
