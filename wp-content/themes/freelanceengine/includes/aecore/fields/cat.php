...
class AE_cat {
	...
	*/
	function render() {
		...
		?>	
		<div class="cat-list-container" id="<?php echo $this->params['taxonomy']; ?>">
			<?php 
				$args = array(
		...
		<?php 
			...
			if(isset($this->params['icons_list']) && $this->params['icons_list']){
				echo '<script type="json/data" class="icon-list" > ' . json_encode($this->params['icons_list']) . '
	            </script>';
			}
