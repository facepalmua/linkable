	...
	if ( $object ) {
		...
		if ( $query->have_posts() ) {
			...
			if ( ! isset( $args['json_id'] ) ) {
				$args['json_id'] = 'ae-' . $post_type . '-json';
			}
...
if ( ! function_exists( 'ae_pagination' ) ):
	...
	function ae_pagination( $query, $current = '', $type = 'page', $text = '' ) {
		 ...
		 * posttype args
		 */
		$query_var['post_type']   = isset( $query->query_vars['post_type'] ) ? $query->query_vars['post_type'] : 'post';
		...
		if ( ( $query->max_num_pages <= 1 && ! et_load_mobile() ) || ! $type ) {
			return;
		}
		$style = '';
		...
		if ( $type === 'page' ) {
			...
			echo '<a id="' . $query_var['post_type'] . '-inview" class="inview load-more-post" href="javascript:void(0)">' . $text . '</a>';
		}
	...
	function ae_user_pagination( $query_args = array(), $total, $current = '' ) {
		...
		if ( ! isset( $query_args['paginate'] ) || ( isset( $query_args['paginate'] ) && $query_args['paginate'] == 'page' ) ) {
			...
			if ( $total > $current ) {
				$text = 'load more';
				if ( ! et_load_mobile() ) {
					$text = ( isset( $query_args['text'] ) ) ? $query_args['text'] : __( "Load more", ET_DOMAIN );
				}
				echo '<a id="' . $query_args['type'] . '-inview" class="inview load-more-post" >' . $text . '</a>';
			}
		}
		...
		if ( ! empty( $product_terms ) ) {
			if ( ! is_wp_error( $product_terms ) ) {
				...
				foreach ( $product_terms as $term ) {
					echo '<a class="' . $class . ' secondary-color " href="' . get_post_type_archive_link( PROJECT ) . '?' . $param . '=' . $term->slug . '"">' . $link_before . $term->name . $link_after . '</a>';
					if ( $taxonomy == 'project_category' ) {
						if ( $i < count( $product_terms ) ) {
							echo ", ";
						}
						$i ++;
					}
