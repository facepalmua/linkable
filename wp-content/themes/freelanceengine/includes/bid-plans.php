 ...
 * @author ThanhTu
 */
function fre_button_bid( $project_id ) {
	global $user_ID;
	...
	if ( ! $profile || ! is_numeric( $profile_id ) ) {
		...
		echo '<a href="#" class="fre-normal-btn purchase-bid-btn primary-bg-color" data-project-id="' . $project_id . '">' . __( 'Bid', ET_DOMAIN ) . '</a>';
	} else {
		// Check invite to bid
		if ( ae_get_option( 'invited_to_bid' ) && ! fre_check_invited( $user_ID, $project_id ) ) {
			...
			if ( ! can_user_bid( $user_ID ) ) {
				...
				echo '<a href="#" class="fre-normal-btn primary-bg-color" data-toggle="modal" data-target="' . $target . '">' . __( 'Bid', ET_DOMAIN ) . '</a>';
			}
		}
	}
}