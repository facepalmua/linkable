                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    foreach ( $plugin_groups as $plugin_group_single_name ) {
                        ...
                        if ( $external_url && preg_match( '|^http(s)?://|', $external_url ) ) {
                            $linked_plugin_groups[] = '<a href="' . esc_url( $external_url ) . '" title="' . $plugin_group_single_name . '" target="_blank" rel="noopener noreferrer">' . $plugin_group_single_name . '</a>';
                        }
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( $external_url && preg_match( '|^http(s)?://|', $external_url ) ) {
                    $table_data[$i]['plugin'] = '<strong><a href="' . esc_url( $external_url ) . '" title="' . $plugin['name'] . '" target="_blank" rel="noopener noreferrer">' . $plugin['name'] . '</a></strong>';
                }
                elseif ( ! $source || preg_match( '|^http://wordpress.org/extend/plugins/|', $source ) ) {
                    $url = add_query_arg(
                        array(
 ...
 * @since 2.2.0
 */
add_action( 'admin_init', 'et_tgmpa_load_bulk_installer' );
if(! function_exists( 'et_tgmpa_load_bulk_installer' )){
    function et_tgmpa_load_bulk_installer() {
