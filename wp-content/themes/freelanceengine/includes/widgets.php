...
class FRE_Social_Widget extends WP_Widget {
    ...
    function __construct() {
        ...
        parent::__construct( 'fre-social', 'FRE Social', $widget_ops );
    }
    ...
    function widget( $args, $instance ) {
        ...
        echo $before_widget;
        echo $before_title;
        echo isset($instance['title']) ? $instance['title'] : ''; // Can set this with a widget option, or omit altogether
        echo $after_title;
    ?>
        <ul class="social-list-footer">
            <?php
            if( ae_get_option('site_facebook') ) {
                echo '<li class="fre-fb-item"><a target="_blank" rel="noopener noreferrer" href="'. ae_get_option('site_facebook') .'"><span><i class="fa fa-facebook"></i></span>'.__("Facebook", ET_DOMAIN).'</a></li>' ;
            }
            if( ae_get_option('site_twitter') ) {
                echo '<li class="fre-twitter-link"><a target="_blank" rel="noopener noreferrer" href="'. ae_get_option('site_twitter') .'"><span><i class="fa fa-twitter"></i></span>'. __("Twitter", ET_DOMAIN).'</a></li>' ;
            }
            if( ae_get_option('site_google') ) {
                echo '<li class="fre-gp-link"><a target="_blank" rel="noopener noreferrer" href="'. ae_get_option('site_google') .'"><span><i class="fa fa-google-plus"></i></span>'.__("Google+", ET_DOMAIN).'</a></li>' ;
            }
            if( ae_get_option('site_linkedin') ) {
                echo '<li class="fre-linkedin-link"><a target="_blank" rel="noopener noreferrer" href="'. ae_get_option('site_linkedin') .'"><span><i class="fa fa-linkedin"></i></span>'.__("Linkedin", ET_DOMAIN).'</a></li>' ;
            }

            do_action('fre_after_social_links');
            ?>
